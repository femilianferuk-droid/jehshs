...
Исправленная версия с корректной отправкой уведомлений
"""
    ...
    if state == "admin_broadcast":
        ...
        if purchase:
            try:
                ...
                logger.info(f"Уведомление об отклонении отправлено пользователю {purchase[1]} для заказа #{purchase_id}")
            except Exception as e:
                logger.error(f"Ошибка отправки уведомления об отклонении: {e}")
        
        await update.message.reply_text(
            f"✅ Заказ #{purchase_id} отклонен. Пользователь уведомлен.",
            reply_markup=get_admin_keyboard()
        )
...
async def show_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query:
        try:
            ...
            logger.error(f"Ошибка редактирования профиля: {e}")
    ...
    try:
        ...
        if purchase and purchase[3] == "approved":
            ...
            try:
                ...
                logger.info(f"Запрос номера отправлен админу для заказа #{purchase_id}")
            except Exception as e:
    ...
    try:
        ...
        if purchase and purchase[3] == "approved":
            ...
            try:
                ...
                logger.info(f"Запрос кода отправлен админу для заказа #{purchase_id}")
            except Exception as e:
                logger.error(f"Ошибка отправки запроса админу: {e}")
                await query.message.reply_text(
                    "❌ Ошибка при отправке запроса.",
...
async def confirm_broadcast(query):
    ...
    try:
        ...
        status_msg = await query.message.reply_text("📢 Рассылка начата...")
...
async def approve_purchase(query, data):
    ...
    try:
        purchase_id = int(data.split("_")[2])
        logger.info(f"Админ {query.from_user.id} одобряет заказ #{purchase_id}")
        
        # Обновляем статус покупки
        db.update_purchase_status(purchase_id, "approved")
        logger.info(f"Статус заказа #{purchase_id} обновлен на 'approved'")
        ...
        if purchase:
            ...
            try:
                # Отправляем сообщение с кнопками для получения номера и кода
                message_text = (
                ...
                logger.info(f"Попытка отправить уведомление пользователю {user_id}")
                ...
                logger.info(f"✅ Уведомление отправлено пользователю {user_id} о подтверждении заказа #{purchase_id}")
                ...
                try:
                    await query.bot.send_message(
                        chat_id=user_id,
                        text="✅ Ваш платеж подтвержден! Используйте кнопки в боте для получения данных.",
                        reply_markup=get_main_keyboard()
                    )
                    logger.info(f"Простое уведомление отправлено пользователю {user_id}")
                except Exception as e2:
                    logger.error(f"Не удалось отправить никакое уведомление пользователю {user_id}: {str(e2)}")
        else:
            logger.error(f"Заказ #{purchase_id} не найден в базе данных")
        
        # Отправляем новое сообщение админу
        await query.message.reply_text(
            f"✅ Заказ #{purchase_id} одобрен.\n"
            f"Пользователь получил уведомление с кнопками для получения данных.",
            reply_markup=InlineKeyboardMarkup([
        ...
        logger.error(f"Ошибка в approve_purchase: {str(e)}")
        await query.message.reply_text(
            f"❌ Ошибка при одобрении заказа: {str(e)}",
            reply_markup=get_back_to_admin_keyboard()
        )
...
async def reject_purchase(query, data):
    ...
    try:
        purchase_id = int(data.split("_")[2])
        logger.info(f"Админ {query.from_user.id} отклоняет заказ #{purchase_id}")
                        ...
                        if purchase:
                            ...
                            if "номер" in reply_text.lower():
                                ...
                                try:
                                    ...
                                    logger.info(f"Номер отправлен пользователю {purchase[1]} для заказа #{purchase_id}")
                                except Exception as e:
                                ...
                                try:
                                    ...
                                    logger.info(f"Код отправлен пользователю {purchase[1]} для заказа #{purchase_id}")
                                except Exception as e:
...
def main():
    ...
    # Запуск бота
    print("=" * 60)
    print("🤖 БОТ ДЛЯ ПРОДАЖИ АККАУНТОВ ЗАПУЩЕН")
    print("=" * 60)
    print(f"👑 ID администратора: {ADMIN_ID}")
    print("🔑 Для доступа к админ-панели отправьте команду /admin")
    print("=" * 60)
    print("🎯 ПРОЦЕСС РАБОТЫ БОТА:")
    print("1. 👤 Пользователь: 🛒 Купить аккаунт -> Выбор страны -> ✅ Я оплатил(а)")
    print("2. 👤 Пользователь: отправляет скриншот оплаты")
    print("3. 👑 Админ: получает уведомление с кнопками ✅ Одобрить / ❌ Отклонить")
    print("4. 👑 Админ: нажимает ✅ Одобрить")
    print("5. 👤 Пользователь: получает уведомление с кнопками:")
    print("   📱 Получить номер  и  🔢 Получить код")
    print("6. 👤 Пользователь: нажимает 📱 Получить номер")
    print("7. 👑 Админ: получает запрос, отвечает на него номером")
    print("8. 👤 Пользователь: получает номер телефона")
    print("9. 👤 Пользователь: нажимает 🔢 Получить код")
    print("10. 👑 Админ: получает запрос, отвечает на него кодом")
    print("11. 👤 Пользователь: получает код")
    print("=" * 60)
    print("📊 База данных: accounts_bot.db")
