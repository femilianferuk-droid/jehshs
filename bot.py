...
Исправлены кнопки "Получить номер" и "Получить код"
"""
...
def get_approved_purchase_keyboard(purchase_id: int):
    """Клавиатура после подтверждения оплаты - ИСПРАВЛЕНО"""
    return InlineKeyboardMarkup([
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === ИСПРАВЛЕННЫЕ КНОПКИ "ПОЛУЧИТЬ НОМЕР" И "ПОЛУЧИТЬ КОД" ===
        ...
        logger.info(f"Пользователь {user_id} запрашивает номер для заказа #{purchase_id}")
        ...
        purchase_id = int(data.split("_")[2])
        logger.info(f"Пользователь {user_id} запрашивает код для заказа #{purchase_id}")
...
def main():
    ...
    print("   • 📱 Получить номер - работает")
    print("   • 🔢 Получить код - работает")
    print("=" * 60)
