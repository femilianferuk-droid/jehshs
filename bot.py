...
# ПРАЙС-ЛИСТ СТРАН (исправленный согласно предоставленным данным)
COUNTRIES = {
...
def create_countries_keyboard(page: int = 0):
    ...
    for i, (code, info) in enumerate(page_countries):
    ...
    if page < total_pages - 1:
        navigation_buttons.append(InlineKeyboardButton("Вперед ➡️", callback_data=f"page_{page+1}"))
...
async def handle_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for code, info in COUNTRIES.items():
        if info['name'] == order_info[4]:  # country_name находится в индексе 4
            country_info = info
            break
...
async def handle_admin_approval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.message.edit_text(f"✅ Заказ #{order_id} одобрен, но не удалось уведомить пользователя: {e}")
...
async def handle_admin_rejection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Ошибка отправки сообщения пользователю: {e}")
        await query.message.edit_text(f"❌ Заказ #{order_id} отклонен, но не удалось уведомить пользователя: {e}")
...
async def handle_data_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data[f'admin_reply_to_{order_id}_{data_type}'] = {
        'user_id': user.id,
        'order_id': order_id,
        'data_type': data_type,
...
async def handle_admin_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key in list(context.user_data.keys()):
        if key.startswith('awaiting_admin_reply_for_'):
            ...
            user_data = context.user_data.get(user_id_key)
...
def main():
    ...
    # Обработчик сообщений от админа (первым, чтобы перехватывать ответы)
    ...
    # Общий обработчик текстовых сообщений (последним)
    application.add_handler(MessageHandler(
    ...
    print(f"💰 Самая дешевая страна: {min(COUNTRIES.items(), key=lambda x: x[1]['price_rub'])[1]['name']} - {min(COUNTRIES.items(), key=lambda x: x[1]['price_rub'])[1]['price_rub']}₽")
    print(f"💎 Самая дорогая страна: {max(COUNTRIES.items(), key=lambda x: x[1]['price_rub'])[1]['name']} - {max(COUNTRIES.items(), key=lambda x: x[1]['price_rub'])[1]['price_rub']}₽")
    print("=" * 50)
