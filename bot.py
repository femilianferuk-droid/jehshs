...
def get_check_purchase_keyboard(purchase_id: int):
    ...
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ Одобрить", callback_data=f"approve_{purchase_id}"),
            InlineKeyboardButton("❌ Отклонить", callback_data=f"reject_{purchase_id}")
        ],
        [InlineKeyboardButton("« Назад", callback_data="back_to_admin_checks")]
    ])
...
async def handle_admin_state(update: Update, context: ContextTypes.DEFAULT_TYPE, state: str, data: str, text: str):
    ...
    if state == "admin_broadcast":
        ...
        await update.message.reply_text(
            ...
            f"Отправить всем пользователям? (да/нет/отмена)",
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "back_to_admin_checks":
        if user_id == ADMIN_ID:
...
async def reject_purchase(query, data):
    ...
    await query.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("« Отмена", callback_data="back_to_admin_checks")]
        ])
    )
...
async def handle_admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.reply_to_message:
        ...
        if user_id == ADMIN_ID:
            ...
            # Проверяем, является ли это ответом на отклонение чека
            elif "Введите причину отклонения" in reply_text:
                # Извлекаем ID покупки из состояния
                state_data = db.get_user_state(user_id)
                if state_data and state_data[0].startswith("admin_reject_"):
                    purchase_id = int(state_data[0].split("_")[2])
