...
class Database:
    ...
    def add_issued_data(self, order_id: str, data_type: str, data_text: str = None, file_path: str = None):
        """Добавление выданных данных"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO issued_data (order_id, data_type, data_text, file_path) VALUES (?, ?, ?, ?)",
                (order_id, data_type, data_text, file_path)
            )
            conn.commit()
            return cursor.lastrowid
    ...
    def get_issued_data(self, order_id: str, data_type: str = None) -> list:
        ...
        with self.get_connection() as conn:
            ...
            if data_type:
                cursor.execute(
                    "SELECT data_text, file_path FROM issued_data WHERE order_id = ? AND data_type = ? ORDER BY issued_at DESC LIMIT 1",
                    (order_id, data_type)
                )
            else:
                cursor.execute(
                    "SELECT data_type, data_text, file_path FROM issued_data WHERE order_id = ? ORDER BY issued_at DESC",
                    (order_id,)
                )
            return cursor.fetchall()
...
def create_balance_pending_keyboard():
    ...
    for transaction in pending_transactions:
        button_text = f"💳 {transaction['username'] or transaction['user_id']} - {transaction['amount']}₽"
        button = InlineKeyboardButton(
            button_text, 
...
async def process_balance_amount(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    try:
        ...
        payment_text = (
            ...
            f"ID транзакции: BAL-{transaction_id}\n\n"
            f"⚠️ После оплаты отправьте скриншот чека (фото или документ)."
        )
...
async def handle_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if 'waiting_screenshot_for' in context.user_data:
            ...
            # Это обрабатывается отдельной функцией
            await handle_balance_screenshot(update, context)
            return
        else:
...
async def handle_admin_approval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.message.edit_text(admin_notification)
...
async def handle_data_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if issued_data:
            ...
            data_text, file_path = issued_data[0]
                    ...
                    try:
                        with open(file_path, 'rb') as file:
                            await context.bot.send_document(
                                chat_id=user.id,
                                document=file,
                                caption=f"💾 TData для заказа #{order_id}"
                            )
                        # Показываем дополнительные кнопки
                        await show_additional_buttons(context, order_id, user.id)
                        return
                    except Exception as e:
                        logger.error(f"Ошибка отправки файла: {e}")
                        message_text += f"💾 TData: Файл не найден\n"
                else:
                    message_text += f"💾 TData: Файл не найден\n"
            elif data_type == "2fa":
        ...
        if data_type == "tdata":
            admin_text += "\n\n📎 Отправьте файл с TData"
        else:
            admin_text += "\n\n💬 Отправьте текстовое сообщение с данными"
...
async def show_additional_buttons(context: ContextTypes.DEFAULT_TYPE, order_id: str, user_id: int):
    """Показать дополнительные кнопки после получения данных"""
    ...
    if keyboard:
        ...
        await context.bot.send_message(
            chat_id=user_id,
            text="📋 Вы можете получить дополнительные данные:",
            reply_markup=reply_markup
        )
...
async def handle_prize_claim(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not db.can_claim_prize(user.id):
            await query.message.edit_text(
                "⏰ Вы уже получали приз сегодня. Приходите через 24 часа!",
                reply_markup=create_prize_keyboard(False)
            )
            return
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "admin_stats":
            ...
            await show_admin_stats_refresh(update, context)
...
async def show_admin_stats_refresh(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def handle_admin_reply_request(query, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) >= 4:
        ...
        if data_type == "tdata":
            await query.message.edit_text(
                f"💬 Отправьте файл с TData для заказа #{order_id}:\n\n"
                f"📎 Пожалуйста, отправьте файл (документ)."
            )
        else:
            data_type_names = {
                "phone": "номер телефона",
            ...
            await query.message.edit_text(
                f"💬 Введите {data_type_names.get(data_type, 'данные')} для заказа #{order_id}:\n\n"
                f"📝 Отправьте текстовое сообщение с данными."
            )

async def process_admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str = None):
    ...
    try:
        ...
        if data_type == "tdata":
            if update.message.document:
                ...
                # Сохраняем файл
                os.makedirs("tdata_files", exist_ok=True)
                file_path = f"tdata_files/{order_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
                await file.download_to_drive(file_path)
                
                # Сохраняем в БД
                db.add_issued_data(order_id, data_type, "TData файл", file_path)
                
                # Отправляем файл пользователю
                try:
                    with open(file_path, 'rb') as tdata_file:
                        await context.bot.send_document(
                            chat_id=user_id,
                            document=tdata_file,
                            caption=f"💾 TData для заказа #{order_id}"
                ...
                await update.message.reply_text("❌ Пожалуйста, отправьте файл для TData.")
                return
        else:
            # Для других типов данных - текст
            if not text:
                text = update.message.text
        ...
        if message_sent:
            # Показываем дополнительные кнопки пользователю
            await show_additional_buttons(context, order_id, user_id)
...
def main():
    ...
    # Обработчик медиафайлов (скриншоты)
    application.add_handler(MessageHandler(
        filters.PHOTO | filters.Document.ALL,
        handle_screenshot
    ))
    
    # Обработчик документов для админа (TData файлы)
    application.add_handler(MessageHandler(
    ...
    print("✅ Убраны комментарии к переводу")
    print("=" * 50)
    ...
    print("✅ Кнопки 'Одобрить' и 'Отклонить' работают")
    print("=" * 50)
