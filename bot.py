...
async def handle_data_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context.user_data[f'awaiting_admin_reply_{order_id}_{data_type}'] = {
            'user_id': user.id,
            'data_type': data_type,
...
async def handle_admin_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем, не ждем ли мы ввод данных от админа
    if 'awaiting_admin_input' in context.user_data:
        data = context.user_data['awaiting_admin_input']
        order_id = data['order_id']
        data_type = data['data_type']
        
        # Находим информацию о пользователе из контекста
        context_key = f'awaiting_admin_reply_{order_id}_{data_type}'
        if context_key in context.user_data:
            user_data = context.user_data[context_key]
            ...
            try:
                ...
                # Обновляем оригинальное сообщение админа
                try:
                    await context.bot.edit_message_text(
                        chat_id=ADMIN_ID,
                        message_id=data.get('original_message_id'),
                        text=f"✅ Вы отправили {'номер телефона' if data_type == 'phone' else 'код'} для заказа #{order_id}"
                    )
                except:
                    pass
            ...
            await update.message.reply_text("❌ Информация о запросе не найдена.")
            context.user_data.pop('awaiting_admin_input', None)
