...
def main():
    ...
    # Обработчик медиафайлов (скриншоты и файлы TData)
    ...
    async def handle_admin_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.effective_user.id == ADMIN_ID and 'admin_state' in context.user_data and context.user_data['admin_state'] == WAITING_ADMIN_REPLY:
            data_type = context.user_data.get('admin_reply_type')
            if data_type == "tdata":
