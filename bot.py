...
Исправленная система получения номера и кода через кнопки админа
...
SUPPORT_LINK = "https://t.me/starfizovoi"
...
def get_approved_purchase_keyboard(purchase_id: int):
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📖 Получить номер", callback_data=f"get_number__{purchase_id}")],
        [InlineKeyboardButton("🔄 Получить код", callback_data=f"get_code__{purchase_id}")],
    ])
...
def get_check_purchase_keyboard(purchase_id: int):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✔️ Одобрить", callback_data=f"check_approve__{purchase_id}")],
        [InlineKeyboardButton("❌ Отклонить", callback_data=f"check_reject__{purchase_id}")],
...
def get_send_number_keyboard(purchase_id: int):
    """Клавиатура для отправки номера админом"""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📞 Отправить номер", callback_data=f"send_number__{purchase_id}")],
        [InlineKeyboardButton("« Назад", callback_data="back_to_admin")]
    ])

def get_send_code_keyboard(purchase_id: int):
    """Клавиатура для отправки кода админом"""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔢 Отправить код", callback_data=f"send_code__{purchase_id}")],
        [InlineKeyboardButton("« Назад", callback_data="back_to_admin")]
    ])
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📞 Купить аккаунт":
        ...
        if state_data:
            ...
            # Обработка состояния, когда админ отправляет номер
            elif state.startswith("admin_send_number_"):
                purchase_id = int(state.split("_")[3])
                phone_number = text
                
                # Обновляем номер в базе
                db.update_purchase_data(purchase_id, phone_number=phone_number)
                db.clear_user_state(user_id)
                ...
                if purchase:
                    ...
                    try:
                        ...
                        await update.message.reply_text(f"✅ Номер отправлен покупателю для заказа #{purchase_id}")
                        logger.info(f"✅ Номер отправлен покупателю {buyer_id} для заказа #{purchase_id}")
                    except Exception as e:
                        logger.error(f"❌ Ошибка отправки номера покупателю: {str(e)}")
                        await update.message.reply_text(f"❌ Ошибка отправки номера: {str(e)}")
            
            # Обработка состояния, когда админ отправляет код
            elif state.startswith("admin_send_code_"):
                purchase_id = int(state.split("_")[3])
                code = text
                
                # Обновляем код в базе
                db.update_purchase_data(purchase_id, code=code)
                db.clear_user_state(user_id)
                ...
                if purchase:
                    ...
                    try:
                        ...
                        await update.message.reply_text(f"✅ Код отправлен покупателю для заказа #{purchase_id}")
                        logger.info(f"✅ Код отправлен покупателю {buyer_id} для заказа #{purchase_id}")
                    except Exception as e:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === КНОПКИ "ПОЛУЧИТЬ НОМЕР" И "ПОЛУЧИТЬ КОД" ДЛЯ ПОКУПАТЕЛЯ ===
    elif data.startswith("get_number__"):
        purchase_id = int(data.split("__")[1])
        ...
        if purchase[3] == "approved":
            ...
            admin_message = (
                ...
                f"Страна: {purchase[2]}\n"
                f"Сумма: {purchase[3]}₽\n\n"
                f"Для отправки номера нажмите кнопку ниже"
            )
            ...
            try:
                ...
                admin_msg = await context.bot.send_message(
                    chat_id=ADMIN_ID,
                    text=admin_message,
                    reply_markup=get_send_number_keyboard(purchase_id)
                )
    ...
    elif data.startswith("get_code__"):
        purchase_id = int(data.split("__")[1])
        ...
        if purchase[3] == "approved":
            ...
            admin_message = (
                ...
                f"Страна: {purchase[2]}\n"
                f"Сумма: {purchase[3]}₽\n\n"
                f"Для отправки кода нажмите кнопку ниже"
            )
            ...
            try:
                ...
                admin_msg = await context.bot.send_message(
                    chat_id=ADMIN_ID,
                    text=admin_message,
                    reply_markup=get_send_code_keyboard(purchase_id)
                )
                logger.info(f"✅ Сообщение отправлено админу. ID сообщения: {admin_msg.message_id}")
    ...
    # Кнопки админа для отправки номера/кода
    elif data.startswith("send_number__"):
        purchase_id = int(data.split("__")[1])
        if user_id == ADMIN_ID:
            db.set_user_state(user_id, f"admin_send_number_{purchase_id}", "")
            await query.message.reply_text(f"Введите номер телефона для заказа #{purchase_id}:")
    
    elif data.startswith("send_code__"):
        purchase_id = int(data.split("__")[1])
        if user_id == ADMIN_ID:
            db.set_user_state(user_id, f"admin_send_code_{purchase_id}", "")
            await query.message.reply_text(f"Введите код для заказа #{purchase_id}:")
...
def main():
    ...
    try:
        ...
        print("  2. ✅ Новая система получения номера/кода через кнопки админа")
        print("  3. ✅ Обновленная ссылка Crypto Bot")
        print("  4. ✅ Поддержка: starfizovoi.t.me")
        print("=" * 60)
        print("📋 ПРОЦЕСС РАБОТЫ СИСТЕМЫ:")
        print("  1. Покупатель нажимает 'Получить номер'")
        print("  2. Админ получает уведомление с кнопкой 'Отправить номер'")
        print("  3. Админ нажимает кнопку и вводит номер")
        print("  4. Покупатель автоматически получает номер")
        print("=" * 60)
