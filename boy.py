...
CRYPTO_BOT_LINK = "http://t.me/send?start=IVKF2M5j4OO5"

# Аккаунты с отлегой (ИСПРАВЛЕНО с "отлего" на "отлегой")
ACCOUNTS_WITH_OTL = {
    "usa_otl": {"name": "🇺🇸 США с отлегой", "otl": "Отлегой #USA-2025", "price_rub": 150, "stock": 5, "code": "+1"},
    "germany_otl": {"name": "🇩🇪 Германия с отлегой", "otl": "Отлегой #GER-2025", "price_rub": 200, "stock": 3, "code": "+49"},
    "france_otl": {"name": "🇫🇷 Франция с отлегой", "otl": "Отлегой #FRA-2025", "price_rub": 180, "stock": 4, "code": "+33"},
    "uk_otl": {"name": "🇬🇧 UK с отлегой", "otl": "Отлегой #UK-2025", "price_rub": 220, "stock": 2, "code": "+44"},
    "spain_otl": {"name": "🇪🇸 Испания с отлегой", "otl": "Отлегой #ESP-2025", "price_rub": 190, "stock": 3, "code": "+34"},
    "italy_otl": {"name": "🇮🇹 Италия с отлегой", "otl": "Отлегой #ITA-2025", "price_rub": 210, "stock": 2, "code": "+39"},
}
...
class Database:
    ...
    def get_all_otl_accounts(self):
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM otl_accounts WHERE is_active = 1 ORDER BY price_rub")
            rows = cursor.fetchall()
            if rows:
                columns = [description[0] for description in cursor.description]
                return [dict(zip(columns, row)) for row in rows]
    ...
    def get_otl_account(self, code: str):
        """Получение аккаунта с отлегой по коду"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM otl_accounts WHERE code = ? AND is_active = 1",
                (code,)
            )
            row = cursor.fetchone()
            if row:
    ...
    def update_otl_account_stock(self, code: str, new_stock: int):
        """Обновление количества аккаунтов с отлегой"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE otl_accounts SET stock = ? WHERE code = ?",
                (new_stock, code)
            )
            conn.commit()
            return cursor.rowcount > 0

    def create_otl_account(self, code: str, name: str, otl: str, price_rub: int, stock: int, phone_code: str):
        """Создание нового аккаунта с отлегой"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """INSERT OR REPLACE INTO otl_accounts 
                (code, name, otl, price_rub, stock, phone_code) 
                VALUES (?, ?, ?, ?, ?, ?)""",
                (code, name, otl, price_rub, stock, phone_code)
            )
            conn.commit()
    ...
    def deactivate_otl_account(self, code: str):
        """Деактивация аккаунта с отлегой"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE otl_accounts SET is_active = 0 WHERE code = ?",
                (code,)
            )
            conn.commit()
            return cursor.rowcount > 0
...
def load_otl_accounts_from_db():
    ...
    for account in accounts:
        ACCOUNTS_WITH_OTL[account['code']] = {
            'name': account['name'],
            'otl': account['otl'],
...
def create_account_types_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🏷️ Аккаунты с отлегой", callback_data="type_otl")],  # ИСПРАВЛЕНО
    ]
    return InlineKeyboardMarkup(keyboard)
...
def create_admin_panel_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🏷️ Управление отлегой", callback_data="admin_otl")],  # ИСПРАВЛЕНО
...
def create_otl_admin_keyboard():
    ...
    for account in accounts:
        ...
        button_text = f"{account['name']} - {account['price_rub']}₽ {stock_status}"
        button = InlineKeyboardButton(button_text, callback_data=f"admin_otl_edit_{account['code']}")
        keyboard.append([button])
    ...
    try:
        ...
        if user.id == ADMIN_ID and 'admin_state' in context.user_data:
            ...
            if state == WAITING_BROADCAST:
                ...
                return
            elif state == WAITING_OTL_ACCOUNT_DETAILS:
                await process_otl_account_add(update, context, text)
                return
...
async def show_order_details(update: Update, context: ContextTypes.DEFAULT_TYPE, country_code: str):
    ...
    try:
        ...
        account_type = "fiz"
        if country_code in ACCOUNTS_WITH_OTL:
            account_type = "otl"
            if country_code not in ACCOUNTS_WITH_OTL:
                await query.message.edit_text("❌ Ошибка: аккаунт не найден.")
                return
            country_info = ACCOUNTS_WITH_OTL[country_code]
            
            # Проверяем наличие
            if country_info['stock'] <= 0:
        ...
        discount_percent = 0
        discount_code = None
        final_price = country_info['price_rub']
        ...
        if 'current_promo' in context.user_data:
            ...
            final_price = int(country_info['price_rub'] * (100 - discount_percent) / 100)
        ...
        context.user_data['current_order'] = {
            ...
            'original_price': country_info['price_rub'],
        ...
        if account_type == "otl":
            order_text += f"├ Отлегой: {country_info['otl']}\n"  # ИСПРАВЛЕНО
            order_text += f"├ Наличие: {country_info['stock']} шт.\n"
        ...
        if discount_percent > 0:
            ...
            order_text += f"├ Изначальная цена: {country_info['price_rub']}₽\n"
...
async def show_payment_crypto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        payment_text = (
            ...
            f"Перейдите по ссылке для оплата: {CRYPTO_BOT_LINK}\n"
...
async def handle_admin_approval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if order and order['account_type'] == 'otl':
            db_account = db.get_otl_account(order['country_code'])
            if db_account:
                new_stock = max(0, db_account['stock'] - 1)
                db.update_otl_account_stock(order['country_code'], new_stock)
                # Обновляем локальный кэш
                if order['country_code'] in ACCOUNTS_WITH_OTL:
                    ACCOUNTS_WITH_OTL[order['country_code']]['stock'] = new_stock
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "admin_stats":
            ...
            country_code = data.replace("admin_otl_edit_", "")
            await edit_otl_account(update, context, country_code)
        elif data == "admin_otl_add":
            await add_otl_account(update, context)
...
async def edit_otl_account(update: Update, context: ContextTypes.DEFAULT_TYPE, country_code: str):
    """Редактировать аккаунт с отлегой"""
    query = update.callback_query

    account = db.get_otl_account(country_code)
    if not account:
    ...
    edit_text = (
        ...
        f"Страна: {account['name']}\n"
        f"Отлегой: {account['otl']}\n"
        f"Цена: {account['price_rub']}₽\n"
        f"Наличие: {account['stock']} шт.\n"
        f"Код: {account['phone_code']}\n\n"
    ...
    context.user_data['admin_state'] = WAITING_OTL_ACCOUNT_DETAILS
    context.user_data['otl_edit_code'] = country_code

    await query.message.edit_text(edit_text)

async def add_otl_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Добавить новый аккаунт с отлегой"""
    query = update.callback_query

    add_text = (
        "➕ Добавление аккаунта с отлегой\n\n"
        "Введите данные в формате:\n"
        "`КОД_СТРАНЫ НАЗВАНИЕ ОТЛЕГОЙ ЦЕНА НАЛИЧИЕ ТЕЛЕФОННЫЙ_КОД`\n\n"
        "Пример: `usa_otl2 🇺🇸 США с отлегой Отлегой #USA-2026 200 3 +1`\n\n"
        "Для отмены нажмите /admin"
    )

    context.user_data['admin_state'] = WAITING_OTL_ACCOUNT_DETAILS
    context.user_data['otl_edit_code'] = "new"

    await query.message.edit_text(add_text)

async def process_otl_account_add(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработать добавление/редактирование аккаунта с отлегой"""
    user = update.effective_user
    ...
    try:
        country_code = context.user_data.get('otl_edit_code')
        
        if country_code == "new":
            # Добавление нового аккаунта
            parts = text.split(maxsplit=5)
            if len(parts) < 6:
                await update.message.reply_text("❌ Неверный формат. Нужно 6 параметров.")
                return
            ...
            db.create_otl_account(new_code, name, otl, price, stock, phone_code)
            
            # Обновляем локальный кэш
            load_otl_accounts_from_db()
            
            action_text = f"✅ Добавлен новый аккаунт с отлегой: {name}"
            
        else:
            # Редактирование существующего аккаунта
            parts = text.split()
            if len(parts) < 2:
                await update.message.reply_text("❌ Неверный формат. Нужно минимум 2 параметра.")
                return
            ...
            if account:
                ...
                action_text = f"✅ Аккаунт с отлегой {account['name']} обновлен"
            else:
        ...
        # Сбрасываем состояние
        context.user_data.pop('admin_state', None)
        context.user_data.pop('otl_edit_code', None)
        ...
        logger.error(f"Ошибка обработки аккаунта с отлегой: {e}")
...
def main():
    ...
    print("=" * 50)
    print(f"🏷️ Аккаунты с отлегой:")
    accounts = db.get_all_otl_accounts()
    for account in accounts:
        stock_status = "✅" if account['stock'] > 0 else "❌"
        print(f"   {account['name']} - {account['price_rub']}₽ {stock_status} ({account['stock']} шт.)")
    print("=" * 50)
