...
def create_data_request_keyboard(order_id: str, has_code: bool = False, has_2fa: bool = False):
    """Создание клавиатуры для запроса данных"""
    keyboard = []
    
    # Проверяем, какие данные уже были запрошены
    issued_data = db.get_issued_data(order_id)
    issued_types = [data[0] for data in issued_data]
    
    if "phone" not in issued_data:
        keyboard.append([InlineKeyboardButton("📱 Получить номер", callback_data=f"get_num_{order_id}")])
    
    if "phone" in issued_types and "code" not in issued_types:
        keyboard.append([InlineKeyboardButton("🔑 Получить код", callback_data=f"get_code_{order_id}")])
    
    if "code" in issued_types and "2fa" not in issued_types:
...
async def handle_data_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if issued_data:
            ...
            # Получаем клавиатуру с учетом полученных данных
            keyboard = create_data_request_keyboard(order_id)
...
async def process_broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    # Проверяем, есть ли активный заказ на рассылку
    order_id = context.user_data.get('broadcast_order_id')
    user_id = context.user_data.get('broadcast_user_id')
    
    if not order_id or user.id != user_id:
        await update.message.reply_text("❌ Ошибка: не найден активный заказ на рассылку.")
        return
    
    # Сохраняем сообщение
    context.user_data['broadcast_message'] = text
    
    # Просим отправить файл с чатами
    context.user_data['admin_state'] = WAITING_BROADCAST_FILE
    ...
    await update.message.reply_text(
        ...
        "@chat2\n"
        "https://t.me/chat3\n\n"
        "Или отправьте ссылку на папку с чатами."
    )
...
async def handle_broadcast_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if update.message.document:
            ...
            # Это текст или ссылка
            if update.message.text.startswith('http'):
                # Ссылка на папку
                chat_list = [update.message.text]
            else:
                # Текстовый список
        ...
        context.user_data['broadcast_chats'] = [chat.strip() for chat in chat_list if chat.strip()]
        
        # Просим ввести задержку
        context.user_data['admin_state'] = WAITING_BROADCAST_DELAY
        
        await update.message.reply_text(
            f"✅ Получен список из {len(context.user_data['broadcast_chats'])} чатов.\n\n"
...
async def process_broadcast_delay(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    try:
        ...
        hours = context.user_data.get('broadcast_hours', 1)
        
        if not message or not chats:
            await update.message.reply_text("❌ Ошибка: не найдены данные для рассылки.")
            return
        ...
        await update.message.reply_text(
            f"🚀 Запускаю рассылку...\n\n"
            f"📊 Параметры:\n"
            f"├ Часов рассылки: {hours}\n"
            f"├ Сообщений: {len(chats)}\n"
            f"├ Задержка: {delay} секунд\n"
            f"└ Общее время: ~{len(chats) * delay / 3600:.1f} часов\n\n"
            f"⏳ Рассылка началась..."
        )
        
        # Здесь должен быть код для реальной рассылки
        # Вместо этого отправляем информацию пользователю
        await update.message.reply_text(
            ...
            f"└ Статус: Запущена\n\n"
            f"📢 Рассылка будет выполнена автоматически в течение указанного времени."
        )
        ...
        context.user_data.pop('broadcast_hours', None)
        context.user_data.pop('admin_state', None)
...
async def process_admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    try:
        ...
        if order:
            # Создаем клавиатуру с учетом полученных данных
            keyboard = create_data_request_keyboard(order_id)
...
def main():
    ...
    # Обработчик файлов для рассылки
    application.add_handler(MessageHandler(
    ...
    print("📱 Кнопки получения номера/кода/2FA настроены!")
