...
async def handle_data_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Сохраняем информацию в БД для связи
        db.add_issued_data(order_id, f"pending_{data_type}", f"user:{user.id}")
...
async def handle_admin_reply_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка запроса админа на ответ (кнопка Ответить)"""
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        if len(parts) >= 4:
            ...
            # Сохраняем информацию в БД для отслеживания
            db.add_issued_data(order_id, f"awaiting_{data_type}", f"admin_request")
            
            # Сохраняем информацию в контексте для быстрого доступа
...
async def handle_admin_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Вариант 1: Проверяем контекст в памяти
    if 'awaiting_admin_input' in context.user_data:
        data = context.user_data['awaiting_admin_input']
        order_id = data['order_id']
        data_type = data['data_type']
        
        # Ищем информацию о пользователе из БД
        pending_requests = db.get_issued_data(order_id, f"pending_{data_type}")
        if pending_requests:
            # Извлекаем user_id из записи в БД
            pending_record = pending_requests[0][0]
            if pending_record.startswith("user:"):
                user_id = int(pending_record.split(":")[1])
                ...
                try:
                    ...
                    # Сохраняем фактические данные в БД
                    ...
                    with db.get_connection() as conn:
                        cursor = conn.cursor()
                        cursor.execute(
                            "DELETE FROM issued_data WHERE order_id = ? AND data_type LIKE ?",
                            (order_id, f"%{data_type}%")
                        )
                        conn.commit()
                    
                    # Добавляем окончательную запись
                    db.add_issued_data(order_id, data_type, text)
    ...
    # Вариант 2: Ищем запросы в БД
    with db.get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT order_id, data_type FROM issued_data WHERE data_text = 'admin_request'"
        )
        pending_requests = cursor.fetchall()
        
        for order_id_db, data_type_db in pending_requests:
            # Ищем user_id для этого заказа
            cursor.execute(
                "SELECT data_text FROM issued_data WHERE order_id = ? AND data_type LIKE ?",
                (order_id_db, f"pending%")
            )
            user_record = cursor.fetchone()
            
            if user_record and user_record[0].startswith("user:"):
                user_id = int(user_record[0].split(":")[1])
                
                # Отправляем данные пользователю
                try:
                    await context.bot.send_message(
                        chat_id=user_id,
                        text=f"📱 Данные для заказа #{order_id_db}:\n\n"
                             f"{'📞 Номер телефона' if 'phone' in data_type_db else '🔑 Код'}: `{text}`\n\n"
                             f"⚠️ Сохраните эти данные в надежном месте!",
                        parse_mode='Markdown'
                    )
                    
                    # Сохраняем фактические данные в БД
                    actual_data_type = "phone" if "phone" in data_type_db else "code"
                    db.add_issued_data(order_id_db, actual_data_type, text)
                    
                    # Удаляем временные записи
                    cursor.execute(
                        "DELETE FROM issued_data WHERE order_id = ? AND (data_type LIKE ? OR data_type LIKE ?)",
                        (order_id_db, f"%pending%", f"%awaiting%")
                    )
                    conn.commit()
                    
                    # Уведомляем админа
                    await update.message.reply_text(
                        f"✅ Данные отправлены пользователю для заказа #{order_id_db}"
                    )
    ...
    # Если не нашли запрос, просто игнорируем сообщение
    logger.info(f"Админ отправил сообщение, но активных запросов не найдено: {text}")
