...
# ====================== КОНФИГУРАЦИЯ ====================== 
BOT_TOKEN = "8111585379:AAGw-qiiJ4aecIq4q4WUZ3B_Qz5QHovx_aw" 
ADMIN_ID = 7973988177 
...
CRYPTO_BOT_LINK = "http://t.me/send?start=IVKF2M5j40O5"  # ИСПРАВЛЕНА ССЫЛКА
...
(
    ...
    WAITING_OTL_STOCK,
    # Состояния для услуг
    WAITING_RENT_COUNTRY,
    WAITING_RENT_PRICE,
    WAITING_BROADCAST_HOURS,
    WAITING_BROADCAST_MESSAGE,
    WAITING_BROADCAST_FILE,
    WAITING_BROADCAST_DELAY,
) = range(20)
...
class Database:
        ...
        with self.get_connection() as conn: 
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS orders (
                    ...
                    service_type TEXT DEFAULT 'purchase',
                    service_data TEXT,
            ...
            # Таблица для аренды аккаунтов
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS rent_accounts (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    country_code TEXT,
                    country_name TEXT,
                    price_per_hour INTEGER DEFAULT 10,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    is_active BOOLEAN DEFAULT 1
                )
            """)
    ...
    def create_order(self, order_id: str, user_id: int, country_code: str, country_name: str, phone_code: str, price_rub: int, discount_code: str = None, discount_percent: int = 0, account_type: str = "fiz", service_type: str = "purchase", service_data: str = None):
        """Создание нового заказа"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """INSERT INTO orders
                (order_id, user_id, country_code, country_name, phone_code, price_rub, status, discount_code, discount_percent, account_type, service_type, service_data)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (order_id, user_id, country_code, country_name, phone_code, price_rub, 'pending', discount_code, discount_percent, account_type, service_type, service_data)
            )
            conn.commit()
            return order_id
    ...
    def get_user_orders(self, user_id: int, limit: int = 10) -> list:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """SELECT order_id, country_name, price_rub, created_at, status, account_type, service_type
                FROM orders
                WHERE user_id = ?
    ...
    def get_completed_user_orders(self, user_id: int, limit: int = 10) -> list:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """SELECT order_id, country_name, price_rub, created_at, account_type, service_type
                FROM orders
    ...
    def get_all_rent_accounts(self):
        """Получение всех аккаунтов для аренды"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM rent_accounts WHERE is_active = 1 ORDER BY country_name")
            rows = cursor.fetchall()
            if rows:
                columns = [description[0] for description in cursor.description]
                return [dict(zip(columns, row)) for row in rows]
    ...
    def get_rent_account(self, country_code: str):
        """Получение аккаунта для аренды по коду страны"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM rent_accounts WHERE country_code = ? AND is_active = 1",
                (country_code,)
            )
            row = cursor.fetchone()
            if row:
    ...
    def create_rent_account(self, country_code: str, country_name: str, price_per_hour: int):
        """Создание нового аккаунта для аренды"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """INSERT OR REPLACE INTO rent_accounts 
                (country_code, country_name, price_per_hour) 
                VALUES (?, ?, ?)""",
                (country_code, country_name, price_per_hour)
            )
            conn.commit()
    ...
    def update_rent_account(self, country_code: str, price_per_hour: int):
        """Обновление аккаунта для аренды"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE rent_accounts SET price_per_hour = ? WHERE country_code = ?",
                (price_per_hour, country_code)
            )
            conn.commit()
            return cursor.rowcount > 0

    def deactivate_rent_account(self, country_code: str):
        """Деактивация аккаунта для аренды"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE rent_accounts SET is_active = 0 WHERE country_code = ?",
                (country_code,)
            )
            conn.commit()
            return cursor.rowcount > 0
    ...
    def get_statistics(self):
        ...
        with self.get_connection() as conn:
            ...
            # Аккаунты для аренды
            cursor.execute("SELECT COUNT(*) FROM rent_accounts WHERE is_active = 1")
            total_rent_accounts = cursor.fetchone()[0]

            # Заказы на услуги
            cursor.execute("SELECT COUNT(*) FROM orders WHERE service_type IN ('rent', 'broadcast')")
            service_orders = cursor.fetchone()[0]
            ...
            return {
                ...
                'total_rent_accounts': total_rent_accounts,
...
def create_main_keyboard():
    ...
    keyboard = [
        ...
        [KeyboardButton("🔧 Услуги"), KeyboardButton("🎫 Промокод")],
        [KeyboardButton("🎁 Новогодние призы"), KeyboardButton("ℹ️ О нас")],
...
def create_services_keyboard():
    """Создание клавиатуры выбора услуг"""
    keyboard = [
        [InlineKeyboardButton("⏳ Аренда аккаунта (1 час)", callback_data="service_rent")],
        [InlineKeyboardButton("📢 Рассылка", callback_data="service_broadcast")],
        [InlineKeyboardButton("⬅ Назад в меню", callback_data="back_to_main")]
    ]
    return InlineKeyboardMarkup(keyboard)

def create_rent_countries_keyboard(page: int = 0):
    """Создание клавиатуры с пагинацией стран для аренды"""
    countries_list = list(COUNTRIES.items())
    items_per_page = 6
    ...
    for code, info in page_countries:
        # Проверяем, есть ли страна в таблице аренды
        rent_account = db.get_rent_account(code)
        if rent_account:
            button_text = f"{info['name']} - {rent_account['price_per_hour']}₽/час"
        else:
            button_text = f"{info['name']} - Не настроено"
        button = InlineKeyboardButton(button_text, callback_data=f"rent_{code}")
        keyboard.append([button])

    # Кнопки навигации
    navigation_buttons = []
    if page > 0:
        navigation_buttons.append(InlineKeyboardButton("◀ Назад", callback_data=f"rent_page_{page-1}"))
    
    if page < total_pages - 1:
        navigation_buttons.append(InlineKeyboardButton("Вперед ▶", callback_data=f"rent_page_{page+1}"))
    
    if navigation_buttons:
        keyboard.append(navigation_buttons)

    # Кнопка "Назад" к выбору услуг
    keyboard.append([InlineKeyboardButton("⬅ Назад к услугам", callback_data="back_to_services_from_rent")])

    return InlineKeyboardMarkup(keyboard)

def create_broadcast_hours_keyboard():
    """Создание клавиатуры выбора часов рассылки"""
    keyboard = []
    row = []
    for hours in range(1, 25):
        price = hours * 4
        button_text = f"{hours}ч - {price}₽"
        button = InlineKeyboardButton(button_text, callback_data=f"broadcast_{hours}")
        row.append(button)
        if len(row) == 3:
            keyboard.append(row)
            row = []
...
def create_data_request_keyboard(order_id: str, has_phone: bool = False, has_code: bool = False, has_2fa: bool = False):
    """Создание клавиатуры для запроса данных - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
    keyboard = []
    
    # Проверяем, какие данные уже были получены
    issued_data = db.get_issued_data(order_id)
    issued_types = [data[0] for data in issued_data]
    
    has_phone = "phone" in issued_types
    has_code = "code" in issued_types
    has_2fa = "2fa" in issued_types
    
    # Показываем только следующую кнопку в последовательности
    if not has_phone:
        # Если нет номера - показываем кнопку получения номера
        keyboard.append([InlineKeyboardButton("📱 Получить номер", callback_data=f"get_num_{order_id}")])
    elif not has_code:
        # Если есть номер, но нет кода - показываем кнопку получения кода
        keyboard.append([InlineKeyboardButton("🔑 Получить код", callback_data=f"get_code_{order_id}")])
    elif not has_2fa:
        # Если есть код, но нет 2FA - показываем кнопку получения 2FA
        keyboard.append([InlineKeyboardButton("🔐 Получить 2FA", callback_data=f"get_2fa_{order_id}")])
    else:
        # Все данные получены
        keyboard.append([InlineKeyboardButton("✅ Все данные получены", callback_data="no_action")])
...
def create_admin_panel_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🏷️ Управление отлегой", callback_data="admin_otl"),
         InlineKeyboardButton("⏳ Управление арендой", callback_data="admin_rent")],
...
def create_rent_admin_keyboard():
    """Создание клавиатуры управления арендой"""
    keyboard = []
    accounts = db.get_all_rent_accounts()
    for account in accounts:
        button_text = f"{account['country_name']} - {account['price_per_hour']}₽/час"
        button = InlineKeyboardButton(button_text, callback_data=f"admin_rent_edit_{account['country_code']}")
        keyboard.append([button])
    
    keyboard.append([InlineKeyboardButton("➕ Добавить аренду", callback_data="admin_rent_add")])
    keyboard.append([InlineKeyboardButton("⬅ Назад", callback_data="admin_back_to_main")])
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        welcome_text = (
            ...
            "🔧 Услуги - аренда аккаунтов и рассылки\n"
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Проверяем состояние для рассылки (для всех пользователей)
        if 'waiting_broadcast_message' in context.user_data and context.user_data['waiting_broadcast_message']:
            # Это сообщение для рассылки
            await process_broadcast_message(update, context, text)
            return
            
        if 'waiting_broadcast_file' in context.user_data and context.user_data['waiting_broadcast_file']:
            # Это файл или ссылка на чаты для рассылки
            await handle_broadcast_file(update, context)
            return
            
        if 'waiting_broadcast_delay' in context.user_data and context.user_data['waiting_broadcast_delay']:
            # Это задержка для рассылки
            await process_broadcast_delay(update, context, text)
            return
        ...
        if user.id == ADMIN_ID and 'admin_state' in context.user_data:
            ...
            if state == WAITING_BROADCAST:
                ...
                return
            elif state == WAITING_RENT_COUNTRY:
                await process_rent_country(update, context, text)
                return
            elif state == WAITING_RENT_PRICE:
                await process_rent_price(update, context, text)
                return
...
async def show_about(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    about_text = (
        "ℹ️ О нас\n\n"
        "Добро пожаловать в нашего бота для покупки аккаунтов! 🚀\n\n"
        "Мы предоставляем качественные аккаунты для различных целей:\n"
        "• 📱 ФИЗ аккаунты - аккаунты для обычного использования\n"
        "• 🏷️ Аккаунты с отлегой - аккаунты с дополнительными функциями\n"
        "• ⏳ Аренда аккаунтов - временное использование аккаунтов\n"
        "• 📢 Рассылка - массовая отправка сообщений\n\n"
        "🎁 У нас есть система новогодних призов - получайте подарки каждый день!\n"
        "🎫 Используйте промокоды для получения скидок на покупки!\n\n"
...
async def show_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if orders:
            for order in orders:
                order_id, country_name, price_rub, created_at, account_type, service_type = order
                try:
                ...
                profile_text += f"│  • {date_str} | {icon} {country_name} {type_text} (#{order_short_id})\n"
        else:
...
async def show_new_year_prizes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prize_text = (
        ...
        "• 🎫 Промокод на скидку 1-5% (шанс 95%)\n"
        "• 🎫 Промокод на скидку 5-30% (шанс 4.99%)\n"
...
async def show_services(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🔧 Наши услуги:\n\n"
        "• ⏳ Аренда аккаунта (1 час) - арендуйте аккаунт на 1 час\n"
        "• 📢 Рассылка - массовая отправка сообщений в чаты\n\n"
        "Выберите услугу:",
...
async def show_rent_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показать услугу аренды аккаунтов"""
    query = update.callback_query
    await query.answer()

    try:
        reply_markup = create_rent_countries_keyboard(0)
        await query.message.edit_text(
            "⏳ Аренда аккаунта (1 час)\n\n"
            "Выберите страну для аренды:",
            reply_markup=reply_markup
        )
    except Exception as e:
        logger.error(f"Ошибка в show_rent_service: {e}")
        await query.message.edit_text("Произошла ошибка. Попробуйте еще раз.")

async def handle_rent_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка пагинации стран для аренды"""
    query = update.callback_query
    await query.answer()

    try:
        data_parts = query.data.split("_")
        page = int(data_parts[3])  # Исправлено с [2] на [3]
        
        reply_markup = create_rent_countries_keyboard(page)
        await query.message.edit_text(
            "⏳ Аренда аккаунта (1 час)\n\n"
            "Выберите страну для аренды:",
            reply_markup=reply_markup
        ...
        logger.error(f"Ошибка в handle_rent_page: {e}")
        await query.message.edit_text("Произошла ошибка. Попробуйте еще раз.")

async def show_rent_details(update: Update, context: ContextTypes.DEFAULT_TYPE, country_code: str):
    ...
    try:
        ...
        # Проверяем, настроена ли аренда для этой страны
        rent_account = db.get_rent_account(country_code)
        if not rent_account:
            await query.message.edit_text("❌ Аренда для этой страны пока не настроена.")
            return

        country_info = COUNTRIES[country_code]
        price_per_hour = rent_account['price_per_hour']
        total_price = price_per_hour  # 1 час аренды
        ...
        if 'current_promo' in context.user_data:
            ...
            discount_percent = promo['discount']
            discount_code = promo['code']
            final_price = int(total_price * (100 - discount_percent) / 100)
        ...
        order_text = (
            f"📋 Детали заказа (аренда):\n"
            f"├ Услуга: Аренда аккаунта на 1 час\n"
            f"├ Страна: {country_info['name']}\n"
            f"├ Код страны: {country_info['code']}\n"
            f"├ Цена за час: {price_per_hour}₽\n"
            f"├ Общая стоимость: {final_price}₽\n"
        )
        
        if discount_percent > 0:
            order_text += f"├ Скидка: {discount_percent}%\n"
            order_text += f"├ Изначальная цена: {total_price}₽\n"
        ...
        logger.error(f"Ошибка в show_rent_details: {e}")
        await query.message.edit_text("Произошла ошибка при создании заказа.")

async def show_broadcast_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показать услугу рассылки"""
    query = update.callback_query
    await query.answer()

    try:
        reply_markup = create_broadcast_hours_keyboard()
        await query.message.edit_text(
            "📢 Услуга рассылки\n\n"
            "Цена: 4₽ за каждый час рассылки\n"
            "Выберите продолжительность рассылки:",
            reply_markup=reply_markup
        )
    except Exception as e:
        logger.error(f"Ошибка в show_broadcast_service: {e}")
        await query.message.edit_text("Произошла ошибка. Попробуйте еще раз.")

async def handle_broadcast_hours(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка выбора часов для рассылки"""
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        context.user_data['broadcast_hours'] = hours
        context.user_data['broadcast_price'] = total_price
        ...
        context.user_data['current_order'] = {
            'order_id': order_id,
            'country_code': 'broadcast',
            'country_name': 'Рассылка',
            'phone_code': '+0',
            'price_rub': final_price,
            'original_price': total_price,
            'discount_percent': discount_percent,
            'service_type': 'broadcast',
            'service_data': f'{hours} часов'
        }
        ...
        order_text = (
            f"📋 Детали заказа (рассылка):\n"
            f"├ Услуга: Рассылка сообщений\n"
        ...
        logger.error(f"Ошибка в handle_broadcast_hours: {e}")
        await query.message.edit_text("Произошла ошибка при создании заказа.")
...
async def handle_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if order['service_type'] == 'rent':
            admin_text += f"⏳ Аренда аккаунта\n"
        elif order['service_type'] == 'broadcast':
            admin_text += f"📢 Рассылка\n"
        elif order['account_type'] == 'otl':
            admin_text += f"🏷️ Аккаунт с отлегой\n"
        else:
            admin_text += f"📱 ФИЗ аккаунт\n"
        
        admin_text += f"├ Страна: {order['country_name']}\n"
        admin_text += f"└ Сумма: {order['price_rub']}₽"
...
async def handle_admin_approval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка одобрения заказа админом - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        try:
            if order and order['service_type'] == 'broadcast':
                # Для рассылки просим ввести сообщение
                context.user_data['broadcast_order_id'] = order_id
                context.user_data['broadcast_user_id'] = user_id
                context.user_data['waiting_broadcast_message'] = True
                
                await context.bot.send_message(
                    chat_id=user_id,
                    text=f"✅ Ваш платеж по заказу #{order_id} подтвержден!\n\n"
                         f"📢 Теперь отправьте сообщение для рассылки:"
                )
            else:
                # Для других заказов - стандартное сообщение
                keyboard = create_data_request_keyboard(order_id)
                
                await context.bot.send_message(
                    chat_id=user_id,
                    text=f"✅ Ваш платеж по заказу #{order_id} подтвержден!\n\n"
                         f"Нажмите кнопку ниже чтобы получить данные:",
                    reply_markup=keyboard
                )
...
async def handle_data_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        callback_data = query.data
        if "get_num" in callback_data:
            data_type = "phone"
        elif "get_code" in callback_data:
            data_type = "code"
        elif "get_2fa" in callback_data:
            data_type = "2fa"
        else:
            await query.answer("Неизвестный тип данных")
            return
        ...
        # Проверяем последовательность получения данных
        issued_data = db.get_issued_data(order_id)
        issued_types = [data[0] for data in issued_data]
        
        # Проверяем правильную последовательность
        if data_type == "phone" and "phone" in issued_types:
            await query.answer("❌ Номер уже получен!")
            return
        elif data_type == "code" and ("code" in issued_types or "phone" not in issued_types):
            if "code" in issued_types:
                await query.answer("❌ Код уже получен!")
                return
            else:
                await query.answer("❌ Сначала получите номер!")
                return
        elif data_type == "2fa" and ("2fa" in issued_types or "code" not in issued_types):
            if "2fa" in issued_types:
                await query.answer("❌ 2FA уже получен!")
                return
            else:
                await query.answer("❌ Сначала получите код!")
                return

        # Проверяем, не были ли уже выданы данные
        issued_data_for_type = db.get_issued_data(order_id, data_type)
        if issued_data_for_type:
            # Данные уже выданы - отправляем их
            data_text = issued_data_for_type[0][0]
            
            response_text = f"📱 Данные для заказа #{order_id}:\n\n"
            if data_type == "phone":
                response_text += f"📞 Номер телефона: `{data_text}`\n"
            elif data_type == "code":
                response_text += f"🔑 Код: `{data_text}`\n"
            elif data_type == "2fa":
                response_text += f"🔐 2FA код: `{data_text}`\n"
        ...
        if order['service_type'] == 'rent':
            admin_text += f"├ Тип: ⏳ Аренда аккаунта\n"
        elif order['service_type'] == 'broadcast':
            admin_text += f"├ Тип: 📢 Рассылка\n"
        elif order['account_type'] == 'otl':
            admin_text += f"├ Тип: 🏷️ Аккаунт с отлегой\n"
        else:
            admin_text += f"├ Тип: 📱 ФИЗ аккаунт\n"
        
        if data_type == "phone":
            admin_text += f"├ Запрошено: номер телефона\n"
        elif data_type == "code":
            admin_text += f"├ Запрошено: код\n"
        elif data_type == "2fa":
            admin_text += f"├ Запрошено: 2FA код\n"
        ...
        await query.message.edit_text(
            ...
            reply_markup=create_data_request_keyboard(order_id)
        )
    except Exception as e:
...
async def handle_prize_claim(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Генерируем приз согласно новым шансам
        random_value = random.random() * 100  # 0-100
        
        if random_value < 95:  # 95% шанс
            # Промокод на 1-5%
            discount = random.randint(1, 5)
            prize_type = "promo"
        elif random_value < 99.99:  # 4.99% шанс (95 + 4.99 = 99.99)
            # Промокод на 5-30%
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        elif data == "admin_rent":
            await show_rent_admin_menu(update, context)
        ...
        elif data.startswith("admin_rent_edit_"):
            country_code = data.replace("admin_rent_edit_", "")
            await edit_rent_account(update, context, country_code)
        elif data == "admin_rent_add":
            await start_rent_add(update, context)
...
async def show_admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE, refresh=False):
    ...
    stats_text = (
        ...
        f"⏳ Аренда аккаунтов\n"
        f"├ Настроено: {stats['total_rent_accounts']}\n"
        f"└ Заказов услуг: {stats['service_orders']}\n\n"
        f"🎁 Призы\n"
...
async def show_rent_admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показать меню управления арендой"""
    query = update.callback_query

    rent_text = "⏳ Управление арендой аккаунтов\n\nВыберите страну для редактирования:"

    await query.message.edit_text(rent_text, reply_markup=create_rent_admin_keyboard())
...
async def start_rent_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Начало добавления страны для аренды"""
    query = update.callback_query

    context.user_data['admin_state'] = WAITING_RENT_COUNTRY
    context.user_data['rent_data'] = {}

    await query.message.edit_text(
        "⏳ Добавление страны для аренды\n\n"
        "Введите название страны (например: США, Россия):"
    )

async def process_rent_country(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка ввода страны для аренды"""
    user = update.effective_user
    ...
    if not country_found:
        await update.message.reply_text(
            "❌ Страна не найдена. Пожалуйста, введите корректное название страны."
        )
        return
    
    country_code, country_info = country_found
    
    # Проверяем, не настроена ли уже аренда для этой страны
    existing_account = db.get_rent_account(country_code)
    if existing_account:
        await update.message.reply_text(
            f"❌ Аренда для страны {country_info['name']} уже настроена.\n"
            f"Используйте меню редактирования для изменения цены."
        )
        context.user_data.pop('admin_state', None)
        context.user_data.pop('rent_data', None)
        return
    
    # Сохраняем данные страны
    context.user_data['rent_data']['country_code'] = country_code
    context.user_data['rent_data']['country_name'] = country_info['name']
    
    # Переходим к следующему шагу
    context.user_data['admin_state'] = WAITING_RENT_PRICE
    
    await update.message.reply_text(
        f"⏳ Добавление страны для аренды\n\n"
        f"Шаг 1: Страна: {country_info['name']} ✅\n"
        f"Шаг 2: Введите цену за 1 час аренды (в рублях):\n\n"
        f"Пример: `50` (для 50 рублей в час)"
    )

async def process_rent_price(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка ввода цены для аренды"""
    user = update.effective_user
    ...
    try:
        ...
        rent_data = context.user_data['rent_data']
        
        # Создаем запись в БД
        db.create_rent_account(
            rent_data['country_code'],
            rent_data['country_name'],
            price
        )
        ...
        await update.message.reply_text(
            f"✅ Страна для аренды успешно добавлена!\n\n"
            f"📋 Информация:\n"
            f"├ Страна: {rent_data['country_name']}\n"
            f"└ Цена за час: {price}₽\n\n"
            f"Вернуться в админ-панель: /admin"
        )
...
async def edit_rent_account(update: Update, context: ContextTypes.DEFAULT_TYPE, country_code: str):
    """Редактирование цены аренды для страны"""
    query = update.callback_query
    ...
    if not rent_account:
        await query.message.edit_text("❌ Аренда для этой страны не настроена.")
        return

    edit_text = (
        f"⏳ Редактирование аренды для {country_info['name']}\n\n"
        f"📋 Текущая информация:\n"
        f"├ Страна: {country_info['name']}\n"
        f"└ Цена за час: {rent_account['price_per_hour']}₽\n\n"
        f"Введите новую цену за 1 час аренды (в рублях):\n\n"
        f"Для отмены нажмите /admin"
    )

    context.user_data['admin_state'] = WAITING_RENT_PRICE
    context.user_data['rent_edit_country'] = country_code

    await query.message.edit_text(edit_text)

# ========================================= ОБРАБОТКА РАССЫЛКИ ПОЛЬЗОВАТЕЛЯ =========================================

async def process_broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка сообщения для рассылки от пользователя"""
    user = update.effective_user
    
    # Проверяем состояние
    if 'waiting_broadcast_message' not in context.user_data or 'broadcast_order_id' not in context.user_data:
        await update.message.reply_text("❌ Ошибка: не найдено состояние ожидания сообщения.")
        return
        
    order_id = context.user_data.get('broadcast_order_id')
    user_id = context.user_data.get('broadcast_user_id')
    
    if user.id != user_id:
        await update.message.reply_text("❌ Ошибка: это не ваш заказ.")
        return
    
    # Сохраняем сообщение
    context.user_data['broadcast_message'] = text
    
    # Переходим к следующему шагу - получение файла с чатами
    context.user_data.pop('waiting_broadcast_message', None)
    context.user_data['waiting_broadcast_file'] = True
    
    await update.message.reply_text(
        "📝 Сообщение для рассылки сохранено!\n\n"
        "Теперь отправьте файл со списком чатов (TXT файл, каждый чат на новой строке):\n\n"
        "Формат файла:\n"
        "@chat1\n"
        "@chat2\n"
        "https://t.me/chat3\n\n"
        "Или отправьте текстовое сообщение со списком чатов (каждый чат на новой строке)."
    )

async def handle_broadcast_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка файла с чатами для рассылки"""
    try:
        user = update.effective_user
        
        # Проверяем состояние
        if 'waiting_broadcast_file' not in context.user_data or 'broadcast_order_id' not in context.user_data:
            await update.message.reply_text("❌ Ошибка: не найдено состояние ожидания файла.")
            return
        ...
        if update.message.document:
            ...
            chat_list = file_content.decode('utf-8').split('\n')
            logger.info(f"Получен файл с {len(chat_list)} чатами для рассылки #{order_id}")
        elif update.message.text:
            # Это текстовый список
            chat_list = update.message.text.split('\n')
            logger.info(f"Получен текстовый список с {len(chat_list)} чатами для рассылки #{order_id}")
        else:
            await update.message.reply_text("❌ Пожалуйста, отправьте файл или текст со списком чатов.")
            return
        
        # Очищаем от пустых строк и сохраняем
        cleaned_chats = [chat.strip() for chat in chat_list if chat.strip()]
        
        if not cleaned_chats:
            await update.message.reply_text("❌ Список чатов пуст. Пожалуйста, отправьте корректный список.")
            return
        
        # Сохраняем список чатов
        context.user_data['broadcast_chats'] = cleaned_chats
        
        # Переходим к следующему шагу - получение задержки
        context.user_data.pop('waiting_broadcast_file', None)
        context.user_data['waiting_broadcast_delay'] = True
        
        await update.message.reply_text(
            f"✅ Получен список из {len(cleaned_chats)} чатов.\n\n"
            f"Теперь введите время между сообщениями (в секундах):\n"
            f"Минимум: 100 секунд\n"
            f"Максимум: 2000 секунд\n\n"
            f"Пример: `150` (для 150 секунд между сообщениями)"
        )
        
    except Exception as e:
        logger.error(f"Ошибка обработки файла рассылки: {e}")
        await update.message.reply_text("❌ Ошибка при обработке файла. Пожалуйста, попробуйте еще раз.")

async def process_broadcast_delay(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка задержки между сообщениями"""
    try:
        user = update.effective_user
        
        # Проверяем состояние
        if 'waiting_broadcast_delay' not in context.user_data or 'broadcast_order_id' not in context.user_data:
            await update.message.reply_text("❌ Ошибка: не найдено состояние ожидания задержки.")
            return
        ...
        try:
            delay = int(text)
            if delay < 100 or delay > 2000:
                await update.message.reply_text("❌ Задержка должна быть от 100 до 2000 секунд.")
                return
        except ValueError:
            await update.message.reply_text("❌ Пожалуйста, введите целое число от 100 до 2000.")
            return
        
        # Получаем все данные
        message = context.user_data.get('broadcast_message', '')
        chats = context.user_data.get('broadcast_chats', [])
        
        if not message:
            await update.message.reply_text("❌ Ошибка: не найдено сообщение для рассылки.")
            return
            
        if not chats:
            await update.message.reply_text("❌ Ошибка: не найден список чатов.")
            return
        ...
        hours = 1
        if 'service_data' in order and order['service_data']:
            try:
                hours = int(order['service_data'].split()[0])
            except:
                hours = 1
        
        # Запускаем рассылку (симуляция, так как реальная рассылка требует дополнительной логики)
        total_messages = len(chats)
        total_time_seconds = total_messages * delay
        total_time_hours = total_time_seconds / 3600
        
        response_text = (
            f"✅ Данные для рассылки успешно получены!\n\n"
            f"📋 Детали заказа #{order_id}:\n"
            f"├ Продолжительность: {hours} часов\n"
            f"├ Количество чатов: {len(chats)}\n"
            f"├ Задержка между сообщениями: {delay} секунд\n"
            f"├ Примерное время выполнения: ~{total_time_hours:.1f} часов\n"
            f"├ Сообщение: {message[:100]}...\n\n"
            f"🚀 Рассылка будет запущена администратором в ближайшее время.\n"
            f"⏳ Статус выполнения можно будет уточнить у {SUPPORT_USERNAME}"
        )
        
        # Сохраняем информацию о рассылке в БД
        db.add_issued_data(order_id, "broadcast_info", 
                          f"Часы: {hours}, Чатов: {len(chats)}, Задержка: {delay}с")
        db.add_issued_data(order_id, "broadcast_message", message)
        db.add_issued_data(order_id, "broadcast_chats", ",".join(chats[:10] + ['...'] if len(chats) > 10 else chats))
        
        # Отправляем информацию админу
        admin_text = (
            f"📢 Новая рассылка ожидает запуска\n\n"
            f"📋 Заказ: #{order_id}\n"
        ...
        # Очищаем контекст
        context.user_data.pop('broadcast_order_id', None)
        context.user_data.pop('broadcast_user_id', None)
        context.user_data.pop('broadcast_message', None)
        context.user_data.pop('broadcast_chats', None)
        context.user_data.pop('waiting_broadcast_delay', None)
        
        await update.message.reply_text(response_text)
        
    except Exception as e:
        logger.error(f"Ошибка запуска рассылки: {e}")
        await update.message.reply_text("❌ Ошибка при запуске рассылки. Пожалуйста, обратитесь к администратору.")
...
async def handle_admin_reply_request(query, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) >= 4:
        ...
        await query.message.edit_text(
            f"💬 Введите {'номер телефона' if data_type == 'phone' else 'код' if data_type == 'code' else '2FA код'} для заказа #{order_id}:\n\n"
...
async def process_admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    try:
        ...
        if data_type == "phone":
            message_text += f"📞 Номер телефона: `{text}`\n"
        elif data_type == "code":
            message_text += f"🔑 Код: `{text}`\n"
        elif data_type == "2fa":
            message_text += f"🔐 2FA код: `{text}`\n"
        
        message_text += f"\n💾 Сохраните эти данные в надежном месте!"

        # Получаем информацию о заказе для создания правильной клавиатуры
        order = db.get_order_by_id(order_id)
        ...
        if order:
            ...
            # Если не удалось получить заказ, отправляем без клавиатуры
            await context.bot.send_message(
                chat_id=user_id,
                text=message_text,
                parse_mode='Markdown'
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "claim_prize":
            ...
            await show_services(update.message, context)
            return
        elif data == "back_to_services_from_rent":
            await show_services(update.message, context)
            return
        ...
        if data == "type_fiz":
            ...
            country_code = data.replace("otl_country_", "")
            await process_otl_country_selection(update, context, country_code)
        elif data == "service_rent":
            await show_rent_service(update, context)
        elif data == "service_broadcast":
            await show_broadcast_service(update, context)
        elif data.startswith("rent_"):
            country_code = data.replace("rent_", "")
            await show_rent_details(update, context, country_code)
        elif data.startswith("rent_page_"):
            await handle_rent_page(update, context)
        elif data.startswith("broadcast_"):
            await handle_broadcast_hours(update, context)
        elif data == "pay_card":
        ...
        elif data.startswith("get_num_") or data.startswith("get_code_") or data.startswith("get_2fa_"):
...
def main():
    ...
    # Обработчик медиафайлов (скриншотов и файлов для рассылки)
    application.add_handler(MessageHandler(
        filters.PHOTO | filters.Document.ALL,
        handle_screenshot
    ))
    
    # Обработчик файлов для рассылки (документы)
    application.add_handler(MessageHandler(
        filters.Document.ALL,
        handle_broadcast_file
    ))
    ...
    # Загружаем аккаунты для аренды
    rent_accounts = db.get_all_rent_accounts()
    print(f"⏳ Аккаунтов для аренды: {len(rent_accounts)}")
    ...
    print("=" * 50)
    print(f"⏳ Аккаунты для аренды:")
    rent_accounts = db.get_all_rent_accounts()
    if rent_accounts:
        for account in rent_accounts:
            print(f"   {account['country_name']} - {account['price_per_hour']}₽/час")
    else:
        print("   Нет аккаунтов для аренды")
    print("=" * 50)
    print("🎄 Новогодние призы включены с новыми шансами!")
    print("🎫 Система промокодов активирована!")
    print("🔧 Услуги: Аренда и Рассылка добавлены!")
    print("📱 Кнопки получения номера/кода/2FA настроены (правильная последовательность)!")
    print("🏷️ Пошаговое добавление аккаунтов с отлегой настроено!")
    print("⏳ Управление арендой аккаунтов добавлено!")
    print("📢 Рассылка пользователей с файлами настроена!")
    print("ℹ️ Раздел 'О нас' с ссылками добавлен!")
    print("₿ Исправлена ссылка для оплаты криптоботом!")
    print("=" * 50)
    print("🎰 Шансы новогодних призов:")
    print("   • 95% - промокод 1-5%")
    print("   • 4.99% - промокод 5-30%")
    print("   • 0.01% - бесплатный физ аккаунт")
    print("=" * 50)
    ...
    print("✅ Рассылка после оплаты работает правильно!")
    print("✅ Последовательность получения данных исправлена!")
    print("=" * 50)
