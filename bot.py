...
def get_send_number_keyboard(purchase_id: int):
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📞 Отправить номер", callback_data=f"send_number_{purchase_id}")],
        [InlineKeyboardButton("« Назад", callback_data="back_to_admin")]
    ])
...
def get_send_code_keyboard(purchase_id: int):
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔢 Отправить код", callback_data=f"send_code_{purchase_id}")],
        [InlineKeyboardButton("« Назад", callback_data="back_to_admin")]
    ])
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === КНОПКИ "ПОЛУЧИТЬ НОМЕР" И "ПОЛУЧИТЬ КОД" ДЛЯ ПОКУПАТЕЛЯ ===
        ...
        # ИСПРАВЛЕНО: используем [2] вместо [1]
    ...
    elif data.startswith("send_number_"):
        # ИСПРАВЛЕНО: используем [2] вместо [1]
        purchase_id = int(data.split("_")[2])
        if user_id == ADMIN_ID:
