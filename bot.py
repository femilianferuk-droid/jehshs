...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === ИСПРАВЛЕННЫЕ КНОПКИ "ПОЛУЧИТЬ НОМЕР" И "ПОЛУЧИТЬ КОД" ===
        ...
        # ИСПРАВЛЕНО: используем [1] вместо [2]
        purchase_id = int(data.split("_")[1])
...
def main():
    ...
    try:
        ...
        print("  2. ✅ Кнопка 'Получить номер' - РАБОТАЕТ (исправлено)")
        print("  3. ✅ Кнопка 'Получить код' - РАБОТАЕТ (исправлено)")
        print("  4. ✅ Обновленная ссылка Crypto Bot")
        print("  5. ✅ Поддержка: starfizovoi.t.me")
        print("=" * 60)
        print("📋 ПРОЦЕСС РАБОТЫ КНОПОК:")
        print("  1. Покупатель нажимает 'Получить номер'")
        print("  2. Админ получает запрос с данными заказа")
        print("  3. Админ отвечает на сообщение с номером")
