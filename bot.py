...
# Константы для Mini App
MINI_APP_URL = "https://bot_1765919799_3031_femilianferuk.bothost.ru/"  # Заменить на ваш домен
...
class ProductType(Enum):
    ...
    FIZ_USA = "fiz_usa"
    FIZ_INDIA = "fiz_india"
    FIZ_RUSSIA = "fiz_russia"
    FIZ_KAZAKHSTAN = "fiz_kazakhstan"
    OTLEG = "otleg"
...
class Database:
    ...
    def init_db(self):
        ...
        if cursor.fetchone()[0] == 0:
            cursor.execute('''
                INSERT INTO stats (total_purchases, premium_purchases, fiz_purchases, otleg_purchases, total_revenue)
                VALUES (0, 0, 0, 0, 0.0)
            ''')
        ...
        if cursor.fetchone()[0] == 0:
            ...
            cursor.execute('''
                INSERT INTO config (premium_date, premium_month, premium_price, fiz_prices, otleg_accounts)
                VALUES (?, ?, ?, ?, ?)
            ''', (
    ...
    def add_purchase(self, user_id: int, amount: float):
        conn = sqlite3.connect(self.db_file)
        cursor = conn.cursor()
        cursor.execute('''
            UPDATE users 
    ...
    def update_order_status(self, order_id: int, status: str):
        ...
        # Обновляем статистику если заказ подтвержден
        if status == "confirmed":
            cursor.execute("SELECT price, user_id FROM orders WHERE order_id = ?", (order_id,))
            row = cursor.fetchone()
            if row:
                price, user_id = row
                cursor.execute('''
                ...
                cursor.execute("SELECT product_type FROM orders WHERE order_id = ?", (order_id,))
                product_type = cursor.fetchone()[0]
                
                if "premium" in product_type:
...
def get_price_settings_keyboard():
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text=f"🇺🇸 США: {config.fiz_prices['usa']}₽", callback_data="edit_usa")],
            [InlineKeyboardButton(text=f"🇮🇳 Индия: {config.fiz_prices['india']}₽", callback_data="edit_india")],
            [InlineKeyboardButton(text=f"🇷🇺 Россия: {config.fiz_prices['russia']}₽", callback_data="edit_russia")],
            [InlineKeyboardButton(text=f"🇰🇿 Казахстан: {config.fiz_prices['kazakhstan']}₽", callback_data="edit_kazakhstan")],
...
async def choose_premium(callback: types.CallbackQuery, state: FSMContext):
    config = db.get_config()
    description = (
        f"🎫 Аккаунт с премиум\n\n"
...
async def process_purchase(callback: types.CallbackQuery, state: FSMContext):
    ...
    elif callback.data.startswith("buy_fiz_"):
