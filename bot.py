...
REVIEWS_LINK = "https://t.me/reviewsnezeex"  # Добавлена ссылка на отзывы
...
# НОВАЯ КАТЕГОРИЯ: Обычные аккаунты (для админ-добавления)
REGULAR_ACCOUNTS = {}
...
(
    ...
    # Состояния для пошагового добавления аккаунта
    WAITING_OTL_COUNTRY,
    WAITING_OTL_NAME,
    WAITING_OTL_CODE,
    WAITING_OTL_PRICE,
    WAITING_OTL_STOCK,
    # Новые состояния для добавления обычных аккаунтов
    WAITING_ACCOUNT_TYPE,
    WAITING_REGULAR_COUNTRY,
    WAITING_REGULAR_NAME,
    WAITING_REGULAR_CODE,
    WAITING_REGULAR_PRICE,
    WAITING_REGULAR_STOCK,
) = range(17)
...
class Database:
    ...
    def init_db(self):
        ...
        with self.get_connection() as conn: 
            ...
            # НОВАЯ ТАБЛИЦА: Обычные аккаунты (для админ-добавления)
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS regular_accounts (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    country_code TEXT,
                    country_name TEXT,
                    account_name TEXT,
    ...
    # ================================== ОБЫЧНЫЕ АККАУНТЫ (НОВАЯ КАТЕГОРИЯ) ==================================
    
    def get_all_regular_accounts(self):
        """Получение всех обычных аккаунтов"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM regular_accounts WHERE is_active = 1 ORDER BY country_name")
            rows = cursor.fetchall()
            if rows:
                columns = [description[0] for description in cursor.description]
                return [dict(zip(columns, row)) for row in rows]
    ...
    def get_regular_account(self, account_id: int):
        """Получение обычного аккаунта по ID"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM regular_accounts WHERE id = ? AND is_active = 1",
                (account_id,)
            )
            row = cursor.fetchone()
            if row:
    ...
    def get_regular_account_by_code(self, country_code: str):
        """Получение обычного аккаунта по коду страны"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM regular_accounts WHERE country_code = ? AND is_active = 1",
                (country_code,)
            )
            row = cursor.fetchone()
            if row:
    ...
    def update_regular_account_stock(self, account_id: int, new_stock: int):
        """Обновление количества обычных аккаунтов"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE regular_accounts SET stock = ? WHERE id = ?",
                (new_stock, account_id)
            )
            conn.commit()
            return cursor.rowcount > 0

    def create_regular_account(self, country_code: str, country_name: str, account_name: str, phone_code: str, price_rub: int, stock: int):
        """Создание нового обычного аккаунта"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """INSERT INTO regular_accounts 
                (country_code, country_name, account_name, phone_code, price_rub, stock) 
                VALUES (?, ?, ?, ?, ?, ?)""",
                (country_code, country_name, account_name, phone_code, price_rub, stock)
            )
            conn.commit()
    ...
    def update_regular_account(self, account_id: int, price_rub: int = None, stock: int = None):
        """Обновление обычного аккаунта"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            updates = []
            params = []
            ...
            params.append(account_id)
            query = f"UPDATE regular_accounts SET {', '.join(updates)} WHERE id = ?"
            cursor.execute(query, params)
            conn.commit()
            return cursor.rowcount > 0

    def deactivate_regular_account(self, account_id: int):
        """Деактивация обычного аккаунта"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE regular_accounts SET is_active = 0 WHERE id = ?",
                (account_id,)
            )
            conn.commit()
            return cursor.rowcount > 0
    ...
    def get_statistics(self):
        ...
        with self.get_connection() as conn:
            ...
            # Обычные аккаунты
            cursor.execute("SELECT COUNT(*) FROM regular_accounts WHERE is_active = 1")
            total_regular_accounts = cursor.fetchone()[0]

            # Обычные аккаунты в наличии
            cursor.execute("SELECT COUNT(*) FROM regular_accounts WHERE is_active = 1 AND stock > 0")
            available_regular_accounts = cursor.fetchone()[0]

            # Заказы по типам
            cursor.execute("SELECT account_type, COUNT(*) FROM orders GROUP BY account_type")
            ...
            return {
                ...
                'total_regular_accounts': total_regular_accounts,
                'available_regular_accounts': available_regular_accounts,
...
def load_accounts_from_db():
    ...
    global ACCOUNTS_WITH_OTL, REGULAR_ACCOUNTS
    
    # Загрузка аккаунтов с отлегой
    ACCOUNTS_WITH_OTL = {}
    otl_accounts = db.get_all_otl_accounts()
    for account in otl_accounts:
    ...
    # Загрузка обычных аккаунтов
    REGULAR_ACCOUNTS = {}
    regular_accounts = db.get_all_regular_accounts()
    for account in regular_accounts:
        key = f"regular_{account['id']}"
        REGULAR_ACCOUNTS[key] = {
            'id': account['id'],
...
def create_countries_keyboard(page: int = 0, account_type: str = "fiz"):
    ...
    if account_type == "fiz":
        ...
        countries_list = list(REGULAR_ACCOUNTS.items())
        items_per_page = 4
    else:
        countries_list = []
        items_per_page = 6
...
def create_account_types_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📦 Обычные аккаунты", callback_data="type_regular")],  # Новая кнопка
    ]
    return InlineKeyboardMarkup(keyboard)
...
def create_admin_panel_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📦 Управление обычными", callback_data="admin_regular")],  # Новая кнопка
        [InlineKeyboardButton("➕ Добавить аккаунт", callback_data="admin_add_account")],  # Новая кнопка
...
def create_regular_admin_keyboard():
    """Создание клавиатуры управления обычными аккаунтами"""
    keyboard = []
    accounts = db.get_all_regular_accounts()
    for account in accounts:
        stock_status = "✅" if account['stock'] > 0 else "❌"
        button_text = f"{account['account_name']} - {account['price_rub']}₽ {stock_status}"
        button = InlineKeyboardButton(button_text, callback_data=f"admin_regular_edit_{account['id']}")
        keyboard.append([button])
...
def create_account_type_selection_keyboard():
    """Создание клавиатуры выбора типа аккаунта для добавления"""
    keyboard = [
        [InlineKeyboardButton("🏷️ Аккаунт с отлегой", callback_data="add_account_type_otl")],
        [InlineKeyboardButton("📦 Обычный аккаунт", callback_data="add_account_type_regular")],
        [InlineKeyboardButton("⬅ Отмена", callback_data="admin_back_to_main")]
    ]
    return InlineKeyboardMarkup(keyboard)

def create_country_selection_keyboard():
    """Создание клавиатуры выбора страны"""
    keyboard = []
    ...
    for i in range(0, len(countries_list), 2):
        ...
        for j in range(2):
            if i + j < len(countries_list):
                ...
                row.append(InlineKeyboardButton(button_text, callback_data=f"select_country_{code}"))
        if row:
            keyboard.append(row)
...
def create_about_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("⭐ Отзывы", url=REVIEWS_LINK)],  # Добавлена кнопка отзывов
    ...
    try:
        ...
        if user.id == ADMIN_ID and 'admin_state' in context.user_data:
            ...
            if state == WAITING_BROADCAST:
                ...
                return
            elif state == WAITING_ACCOUNT_TYPE:
                await process_account_type_selection(update, context, text)
                return
                ...
                return
            elif state == WAITING_REGULAR_COUNTRY:
                await process_regular_country(update, context, text)
                return
            elif state == WAITING_REGULAR_NAME:
                await process_regular_name(update, context, text)
                return
            elif state == WAITING_REGULAR_CODE:
                await process_regular_code(update, context, text)
                return
            elif state == WAITING_REGULAR_PRICE:
                await process_regular_price(update, context, text)
                return
            elif state == WAITING_REGULAR_STOCK:
                await process_regular_stock(update, context, text)
                return
...
async def show_support(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    support_text = (
        ...
        f"⚡ Скорость ответа: обычно в течение 5-15 минут\n"
        f"🕒 Работаем: 10:00 - 22:00 (МСК)\n\n"
        f"⚠️ Пожалуйста, сразу опишите вашу ситуацию:\n"
        f"• Номер заказа (если есть)\n"
        f"• Проблема, с которой столкнулись\n"
        f"• Скриншоты (если нужны)\n\n"
        f"Это ускорит решение вашего вопроса!"
    )
    ...
    try:
        ...
        if orders:
            for order in orders:
                ...
                account_type_icon = "🏷️" if account_type == "otl" else "📦" if account_type == "regular" else "📱"
...
async def show_new_year_prizes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prize_text = (
        ...
        "• 🎫 Промокод на скидку 5-10% (шанс 4%)\n"
        "• 🎫 Промокод на скидку 10-30% (шанс 0.99%)\n"
...
async def show_regular_countries(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показать список обычных аккаунтов"""
    query = update.callback_query
    await query.answer()

    try:
        reply_markup = create_countries_keyboard(0, "regular")
        await query.message.edit_text(
            "📦 Обычные аккаунты:\n✅ - в наличии\n❌ - нет в наличии",
            reply_markup=reply_markup
        )
    except Exception as e:
        logger.error(f"Ошибка в show_regular_countries: {e}")
        await query.message.edit_text("Произошла ошибка. Попробуйте еще раз.")
...
async def show_order_details(update: Update, context: ContextTypes.DEFAULT_TYPE, country_code: str):
    ...
    try:
        ...
        if country_code.startswith("otl_"):
        ...
        elif country_code.startswith("regular_"):
            account_type = "regular"
            if country_code not in REGULAR_ACCOUNTS:
                await query.message.edit_text("❌ Ошибка: аккаунт не найден.")
                return
            account_info = REGULAR_ACCOUNTS[country_code]
            ...
            country_name = account_info['name']
            phone_code = account_info['code']
            price_rub = account_info['price_rub']
...
async def handle_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if order['account_type'] == 'otl':
            admin_text += f"🏷️ Аккаунт с отлегой\n"
        elif order['account_type'] == 'regular':
            admin_text += f"📦 Обычный аккаунт\n"
        else:
...
async def handle_admin_approval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Уменьшаем количество аккаунтов если это не стандартный ФИЗ
        order = db.get_order_by_id(order_id)
        if order:
            if order['account_type'] == 'otl':
                ...
                # Находим обычный аккаунт по коду страны
                account = db.get_regular_account_by_code(order['country_code'])
                if account:
                    new_stock = max(0, account['stock'] - 1)
                    db.update_regular_account_stock(account['id'], new_stock)
...
async def handle_data_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        admin_text += f"├ Страна: {order['country_name']}\n"
        admin_text += f"└ Запрошено: {'номер телефона' if data_type == 'phone' else 'код'}"
...
async def handle_prize_claim(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Генерируем приз с новыми шансами
        ...
        if random_value < 95:  # 95% шанс
            ...
            prize_type = "promo"
        elif random_value < 99:  # 4% шанс (95-99)
            # Промокод 5-10%
            discount = random.randint(5, 10)
            prize_type = "promo"
        elif random_value < 99.99:  # 0.99% шанс (99-99.99)
            # Промокод 10-30%
        ...
        if prize_type == "promo":
            ...
            prize_text = (
                f"🎉 ВАУ! Вам выпал ДЖЕКПОТ (шанс 0.01%)!\n\n"
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        elif data == "admin_regular":
            await show_regular_admin_menu(update, context)
        elif data == "admin_add_account":
            await start_add_account(update, context)
        ...
        elif data.startswith("admin_regular_edit_"):
            account_id = int(data.replace("admin_regular_edit_", ""))
            await edit_regular_account(update, context, account_id)
        elif data.startswith("add_account_type_"):
            account_type = data.replace("add_account_type_", "")
            await start_add_account_process(update, context, account_type)
        elif data.startswith("select_country_"):
            country_code = data.replace("select_country_", "")
            await process_country_selection(update, context, country_code)
...
async def show_admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE, refresh=False):
    ...
    stats_text = (
        ...
        f"└ Заказов: {stats.get('otl_orders', 0)}\n\n"
        f"📦 Обычные аккаунты\n"
        f"├ Всего: {stats['total_regular_accounts']}\n"
        f"├ В наличии: {stats['available_regular_accounts']}\n"
        f"└ Заказов: {stats.get('regular_orders', 0)}\n\n"
        f"🎁 Призы\n"
...
async def show_regular_admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показать меню управления обычными аккаунтами"""
    query = update.callback_query

    regular_text = "📦 Управление обычными аккаунтами\n\nВыберите аккаунт для редактирования:"

    await query.message.edit_text(regular_text, reply_markup=create_regular_admin_keyboard())
...
async def edit_regular_account(update: Update, context: ContextTypes.DEFAULT_TYPE, account_id: int):
    """Редактирование обычного аккаунта"""
    query = update.callback_query

    account = db.get_regular_account(account_id)
    if not account:
        await query.message.edit_text("❌ Аккаунт не найден.")
        return

    edit_text = (
        f"📦 Редактирование обычного аккаунта\n\n"
        f"📋 Текущая информация:\n"
    ...
    context.user_data['admin_state'] = WAITING_REGULAR_PRICE
    context.user_data['regular_edit_id'] = account_id
...
async def start_add_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Начать процесс добавления нового аккаунта"""
    query = update.callback_query

    await query.message.edit_text(
        "➕ Добавление нового аккаунта\n\n"
        "Выберите тип аккаунта:",
        reply_markup=create_account_type_selection_keyboard()
    )

async def start_add_account_process(update: Update, context: ContextTypes.DEFAULT_TYPE, account_type: str):
    """Начать процесс добавления аккаунта определенного типа"""
    query = update.callback_query

    if account_type == "otl":
        context.user_data['admin_state'] = WAITING_OTL_COUNTRY
        context.user_data['account_data'] = {'type': 'otl'}
        await query.message.edit_text(
            "🏷️ Добавление аккаунта с отлегой\n\n"
            "Шаг 1: Выберите страну из списка ниже:",
            reply_markup=create_country_selection_keyboard()
        )
    elif account_type == "regular":
        context.user_data['admin_state'] = WAITING_REGULAR_COUNTRY
        context.user_data['account_data'] = {'type': 'regular'}
        await query.message.edit_text(
            "📦 Добавление обычного аккаунта\n\n"
            "Шаг 1: Выберите страну из списка ниже:",
            reply_markup=create_country_selection_keyboard()
        )

async def process_country_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, country_code: str):
    ...
    country_info = COUNTRIES[country_code]
    account_type = context.user_data['account_data']['type']
    
    # Сохраняем данные страны
    context.user_data['account_data']['country_code'] = country_code
    context.user_data['account_data']['country_name'] = country_info['name']
    context.user_data['account_data']['phone_code'] = country_info['code']
    ...
    if account_type == "otl":
        ...
        context.user_data['admin_state'] = WAITING_REGULAR_NAME
        await query.message.edit_text(
            f"📦 Добавление обычного аккаунта\n\n"
            f"Шаг 1: Страна: {country_info['name']} ✅\n"
            f"Шаг 2: Введите название аккаунта:\n\n"
            f"Пример: `Премиум аккаунт США` или `USA Premium`"
        )

async def process_account_type_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка выбора типа аккаунта (текстовый ввод)"""
    user = update.effective_user
    ...
    if "отлегой" in text_lower or "otl" in text_lower:
        context.user_data['admin_state'] = WAITING_OTL_COUNTRY
        context.user_data['account_data'] = {'type': 'otl'}
        await update.message.reply_text(
            "🏷️ Добавление аккаунта с отлегой\n\n"
            "Шаг 1: Введите название страны (например: США, Россия):"
        )
    elif "обычный" in text_lower or "regular" in text_lower or "стандарт" in text_lower:
        context.user_data['admin_state'] = WAITING_REGULAR_COUNTRY
        context.user_data['account_data'] = {'type': 'regular'}
        await update.message.reply_text(
            "📦 Добавление обычного аккаунта\n\n"
        ...
        await update.message.reply_text(
            "❌ Не понял тип аккаунта. Используйте кнопки или напишите:\n"
            "- 'отлегой' для аккаунта с отлегой\n"
            "- 'обычный' для обычного аккаунта"
        )

async def process_regular_country(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка ввода страны для обычного аккаунта"""
    user = update.effective_user
...
async def process_regular_name(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка ввода названия обычного аккаунта"""
    user = update.effective_user
    ...
    if not text or len(text) < 2:
        await update.message.reply_text("❌ Название аккаунта должно содержать минимум 2 символа.")
        return

    # Сохраняем название аккаунта
    context.user_data['account_data']['account_name'] = text
    
    # Переходим к следующему шагу
    context.user_data['admin_state'] = WAITING_REGULAR_PRICE
    
    country_name = context.user_data['account_data']['country_name']
    
    await update.message.reply_text(
        f"📦 Добавление обычного аккаунта\n\n"
        f"Шаг 1: Страна: {country_name} ✅\n"
        f"Шаг 2: Название: {text} ✅\n"
        f"Шаг 3: Введите цену в рублях:\n\n"
        f"Пример: `150` (для 150 рублей)"
    )

async def process_regular_price(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка ввода цены для обычного аккаунта"""
    user = update.effective_user
    ...
    try:
        ...
        # Сохраняем цену
        context.user_data['account_data']['price_rub'] = price
        
        # Переходим к следующему шагу
        context.user_data['admin_state'] = WAITING_REGULAR_STOCK
        
        country_name = context.user_data['account_data']['country_name']
        account_name = context.user_data['account_data']['account_name']
        
        await update.message.reply_text(
            f"📦 Добавление обычного аккаунта\n\n"
            f"Шаг 1: Страна: {country_name} ✅\n"
            f"Шаг 2: Название: {account_name} ✅\n"
            f"Шаг 3: Цена: {price}₽ ✅\n"
...
async def process_regular_stock(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка ввода количества для обычного аккаунта"""
    user = update.effective_user
    ...
    try:
        ...
        account_id = db.create_regular_account(
        ...
        await update.message.reply_text(
            f"✅ Обычный аккаунт успешно добавлен!\n\n"
            f"📋 Информация об аккаунте:\n"
            f"├ Название: {account_data['account_name']}\n"
...
# Обработчики для аккаунтов с отлегой (остаются аналогичными)
async def process_otl_country(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка ввода страны для отлегой"""
    user = update.effective_user
...
async def process_otl_name(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    context.user_data['account_data']['otl_name'] = text
...
async def process_otl_price(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка ввода цены для отлегой"""
    user = update.effective_user
    ...
    try:
        ...
        country_name = context.user_data['account_data']['country_name']
        otl_name = context.user_data['account_data']['otl_name']
...
async def process_otl_stock(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    """Обработка ввода количества для отлегой"""
    user = update.effective_user
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        elif data == "type_regular":
            await show_regular_countries(update, context)
...
def main():
    ...
    print(f"📦 Обычных аккаунтов: {len(REGULAR_ACCOUNTS)}")
    ...
    print("=" * 50)
    print(f"📦 Обычные аккаунты в наличии:")
    regular_accounts = db.get_all_regular_accounts()
    if regular_accounts:
        for account in regular_accounts:
            stock_status = "✅" if account['stock'] > 0 else "❌"
            print(f"   {account['account_name']} - {account['price_rub']}₽ {stock_status} ({account['stock']} шт.)")
    else:
        print("   Нет обычных аккаунтов")
    ...
    print("📦 Пошаговое добавление обычных аккаунтов настроено!")
    print("ℹ️ Раздел 'О нас' с ссылками добавлен!")
    print("🆘 Обновленная информация в поддержке!")
    print("💰 Убраны обязательные комментарии к платежам!")
    print("🎰 Обновлены шансы призов:")
    print("   • Промокод 1-5%: 95% шанс")
    print("   • Промокод 5-10%: 4% шанс")
    print("   • Промокод 10-30%: 0.99% шанс")
